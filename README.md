...
- macOS Intel：下载文件名包含 `x86_64` 的 `.dmg`
- macOS Apple Silicon：下载文件名包含 `aarch64` 或 `arm64` 的 `.dmg`
...
如果 macOS 提示“您无法打开应用程序 Skills Hub.app，因为这台 Mac 不支持此应用程序”，通常是 CPU 架构不匹配：例如 Intel Mac 安装了 Apple Silicon/arm64 版本。可以用下面的命令确认本机架构：

```bash
uname -m
```

输出 `x86_64` 表示 Intel Mac，应下载 `x86_64` 安装包；输出 `arm64` 表示 Apple Silicon，应下载 `aarch64` 或 `arm64` 安装包。这个问题不能通过修改权限、移动安装位置或移除隔离属性解决，必须换成匹配架构的安装包。
