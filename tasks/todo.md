...
- 范围：发布流程拆分 macOS Intel 与 macOS Apple Silicon 两个安装包；README 明确用户如何选择安装包和如何诊断架构；升版本并触发 release workflow。
- 非目标：本轮不做 Developer ID 签名/公证，不解决所有 Gatekeeper 首次打开安全提示。
- 设计方向：优先使用原生 GitHub runner 构建对应架构：`macos-15-intel` 产出 Intel/x86_64，`macos-latest` 产出 Apple Silicon/arm64，避免在单 runner 上做复杂交叉构建。
...
- [x] 将发布版本升到 `0.1.3` 并触发 release workflow
- [x] 修复 `macos-13` runner 无法调度问题，改用 `macos-15-intel`
- [x] 将发布版本升到 `0.1.4` 并重新触发 release workflow
- [x] 运行 workflow 配置和文本检查
- [x] 记录 Review / 复盘

## Verify

- release workflow 中存在独立的 macOS Intel job，运行在 `macos-15-intel`。
...
- 结果：release workflow 新增 `macOS Intel` 构建，使用 `macos-15-intel` 和 `--target x86_64-apple-darwin` 产出 Intel `.dmg`。
...
- 结果：`v0.1.3` 的 Intel job 卡在 `Waiting for a runner to pick up this job`，根因是 `macos-13` 不再是当前可用的标准 Intel runner 标签。
- 结果：根据 GitHub-hosted runners 官方文档，Intel macOS 标准 runner 改为 `macos-15-intel`；发布版本升到 `0.1.4`，准备通过 `v0.1.4` tag 重新触发。
...
- 目标：将已经可用的桌面客户端代码合并到 `main`，并更新 README，让普通用户以下载安装客户端为默认使用方式。
- 范围：README 使用说明、当前分支验证、`codex-desktop-client` 到 `main` 的合并。
- 非目标：本轮不继续开发新客户端功能，不要求普通用户本地启动 API、运行 `npm run dev` 或执行 `npm run build`。
- 设计方向：README 把“下载安装包”作为快速开始的主路径；把 npm / Tauri 命令收进维护者或开发者说明，避免误导普通用户。
